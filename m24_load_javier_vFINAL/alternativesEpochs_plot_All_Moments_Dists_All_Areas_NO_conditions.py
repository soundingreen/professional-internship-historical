# -*- coding: utf-8 -*-
"""
Created on Wed Dec  8 16:01:56 2021
...
for a in s_areas:
    ...
    for m in moments:
        ...
        ax.plot(bins[1:],dist2,label='Delay 2 sec 1',color='red',alpha=0.7)
        ax.plot(bins[1:],dist3,label='Delay 2 sec 3',color='green',alpha=0.7)
        ax.set_title(f'Probability distribution of {mom[m]} in {a}\n during All epochs\n All exp \n without conditions')
        ax.legend()
        ax.set_xlabel('units')
        ax.set_ylabel('Probability')
        fsave=(f"D:/m24_load_javier/alternatives/plots/{a}_Dist_No_conditions_Allexp/{m}_{a}_No_conditions_ALlepochs.svg")
        fig.savefig(fsave)
        print(fsave)
        plt.close('all')

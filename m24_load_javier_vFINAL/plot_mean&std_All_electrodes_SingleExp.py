...
directory=actual+f"/Raw_Mean&std_plots/exp{exp_id}"
print(directory)
try:
...
for ch in electrodes[1]:
    ...
    ax.plot(times,s,color='blue',alpha=0.7,label='std')
    ax.plot(times,m,color='red',alpha=0.7,label='mean')
    ax.set_xlim(times[0],times[-1])
    fn.events(ax)
    ax.legend()
    ax.set_xlabel("Time[s]",fontsize=14)
    ax.set_ylabel("Voltage [uV]",fontsize=14)
    ax.set_title(f'Mean and Standard deviation of the electrode {ch}\n Exp {exp+1} ')
    fsave=(f"Raw_Mean&std_plots/exp{exp_id}/Raw_mean&std_LFP_exp-{exp_id}_{ch}.svg")
    fig.savefig(fsave)
    print(fsave)
    plt.close('all')
    #####################

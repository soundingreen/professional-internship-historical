# -*- coding: utf-8 -*-
"""
Created on Mon Dec  6 18:28:24 2021
...
directory=actual+f"/Raw_Zscore2sks_plots/exp{exp_id}"
print(directory)
try:
...
for ch in electrodes[1]:
    mg=np.mean(lfp[ch],axis=0)
    stdg=np.std(lfp[ch],axis=0)
    z=(lfp[ch]-mg)/stdg
    #mz=np.mean(z,axis=0)
    #sz=np.std(z,axis=0)
    k=kurtosis(z,axis=0)
    ...
    ax.plot(times,sk,color='green',alpha=0.7,label='skweness')
    #ax.plot(times,z[0],color='orange',alpha=0.7,label='Example Z-score trial')
    ...
    ax.set_title(f'Excess kurtosis and skewness of time by time z-scores of the electrode {ch}\n Exp {exp+1} ')
    fsave=(f"Raw_Zscore2sks_plots/exp{exp_id}/Raw_tbt_Zscore_sks_LFP_exp-{exp_id}_{ch}.svg")
    fig.savefig(fsave)
    print(fsave)
    plt.close('all')
    #####################

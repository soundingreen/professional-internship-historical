# -*- coding: utf-8 -*-
"""
Created on Sat Dec  4 18:10:01 2021
...
for e in range(len(epochs)):
    for ch in electrodes[1]:
        directory=actual+f"/Hiper/StandardDev/Conditions_plots/exp{exp_id}/epoch{ne[e]}"
        print(directory)
        try:
        ...
        samples=[c1,c2,c3,c4]
        resample=fn.getMomentsSamples(samples,moment='std')
        ...
        ax.set_title(f'Probability distributions of Std by conditions\n electrode{ch}\n During_{ne[e]}_epoch\n Exp id_{exp_id}')
        ax.set_xlabel('uV')
        ax.set_ylabel('Probability')
        ax.legend()
        fsave=(f"Hiper/StandardDev/Conditions_plots/exp{exp_id}/epoch{ne[e]}/SingleCh_Std_Conditions_epoch{ne[e]}_{ch}.svg")
        fig.savefig(fsave)
        print(fsave)
        plt.close('all')

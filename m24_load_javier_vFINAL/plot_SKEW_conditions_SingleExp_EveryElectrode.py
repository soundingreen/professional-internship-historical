...
for e in range(len(epochs)):
    for ch in electrodes[1]:
        directory=actual+f"/Hiper/Skweness/Conditions_plots/exp{exp_id}/epoch{ne[e]}"
        print(directory)
        try:
        ...
        ax.set_title(f'Probability distributions of Skewness by conditions\n electrode{ch}\n During_{ne[e]}_epoch\n Exp id_{exp_id}')
        ax.set_xlabel('uV')
        ax.set_ylabel('Probability')
        ax.legend()
        fsave=(f"Hiper/Skweness/Conditions_plots/exp{exp_id}/epoch{ne[e]}/SingleCh_Skewness_Conditions_epoch{ne[e]}_{ch}.svg")
        fig.savefig(fsave)
        print(fsave)
        plt.close('all')

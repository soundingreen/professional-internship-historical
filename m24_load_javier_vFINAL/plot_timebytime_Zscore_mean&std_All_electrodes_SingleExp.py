...
directory=actual+f"/Raw_Zscore2_plots/exp{exp_id}"
print(directory)
try:
...
for ch in electrodes[1]:
    ...
    ax.plot(times,mz,color='blue',alpha=0.7,label='meanz')
    ...
    ax.set_title(f'Mean and Std of time by time z-scores of the electrode {ch}\n Exp {exp+1} ')
    fsave=(f"Raw_Zscore2_plots/exp{exp_id}/Raw_tbt_Zscore_LFP_exp-{exp_id}_{ch}.svg")
    fig.savefig(fsave)
    print(fsave)
    plt.close('all')
    #####################

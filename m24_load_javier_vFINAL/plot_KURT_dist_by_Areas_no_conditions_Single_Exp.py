# -*- coding: utf-8 -*-
"""
Created on Tue Dec  7 16:57:18 2021
...
for area in areas:
    directory=actual+f"/KUR_No_conditions_Join_e_plots/exp{exp_id}/All_epochs"
    print(directory)
    try:
    ...
    ax.set_title(f'Probability distribution of Excess Kurtosis in the area {area}\n during All epochs\n Exp id_{exp_id} \n without conditions')
    ax.legend()
    ax.set_xlabel('units')
    ax.set_ylabel('Probability')
    fsave=(f"KUR_No_conditions_Join_e_plots/exp{exp_id}/All_epochs/SingleCh_Kur_No_conditions_Area{area}.svg")
    fig.savefig(fsave)
    print(fsave)
    plt.close('all')

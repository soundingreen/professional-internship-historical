# -*- coding: utf-8 -*-
"""
Created on Mon Dec  6 22:33:38 2021
...
for e in range(len(epochs)):
    for ch in electrodes[1]:
        directory=actual+f"/K_No_conditions_plots/exp{exp_id}/epoch{ne[e]}"
        print(directory)
        try:
        ...
        k=kurtosis(x,axis=1)
        print(k.shape)
        dist,bins=fn.getSampleDistribution(k,nbins=30)
        ax.plot(bins[1:],dist)
        ax.set_title(f'Probability distribution of Kurtosis in the electrode{ch}\n during_{ne[e]}_epoch\n Exp id_{exp_id} \n without conditions')
        ax.set_xlabel('uV')
        ax.set_ylabel('Probability')
        fsave=(f"K_No_conditions_plots/exp{exp_id}/epoch{ne[e]}/SingleCh_Kur_No_conditions_epoch{ne[e]}_{ch}.svg")
        fig.savefig(fsave)
        print(fsave)
        plt.close('all')
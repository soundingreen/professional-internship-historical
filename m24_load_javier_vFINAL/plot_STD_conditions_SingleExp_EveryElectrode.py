...
for e in range(len(epochs)):
    for ch in electrodes[1]:
        directory=actual+f"/Hiper/StandardDev/exp{exp_id}/epoch{ne[e]}"
        print(directory)
        try:
        ...
        cdist,bins=fn.getSamplesDistributions(resample,nbins=6)
        ...
        ax.legend()
        fsave=(f"Hiper/StandardDev/exp{exp_id}/epoch{ne[e]}/SingleCh_Std_Conditions_epoch{ne[e]}_{ch}.svg")
        fig.savefig(fsave)
        print(fsave)
        plt.close('all')
